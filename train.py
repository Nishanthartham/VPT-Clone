...
def setup(args):
    ...
    # setup dist
    print("setting dist path")
    # cfg.DIST_INIT_PATH = "tcp://{}:12399".format("xulab-gpu1.pc.cc.cmu.edu")
    # cfg.DIST_INIT_PATH = "tcp://{}:12399".format(os.environ["SLURMD_NODENAME"])
    print("CFG distributed setup done")
    # setup output dir
    # output_dir / data_name / feature_name / lr_wd / run1
    output_dir = cfg.OUTPUT_DIR
    print(f'output dir = {cfg.OUTPUT_DIR}')
    print(f'datat dir = {cfg.DATA.DATAPATH}')
    lr = cfg.SOLVER.BASE_LR
    wd = cfg.SOLVER.WEIGHT_DECAY
    output_folder = os.path.join(
        cfg.DATA.NAME, cfg.DATA.FEATURE, f"lr{lr}_wd{wd}")
    print(f'final output dir = {output_folder}')

    # train cfg.RUN_N_TIMES times
    count = 1
    while count <= cfg.RUN_N_TIMES:
        print(f"entered into while {count} and run_n_times = {cfg.RUN_N_TIMES}")
...
def get_loaders(cfg, logger):
    ...
    if cfg.DATA.NAME.startswith("vtab-"):
        logger.info("This is VTAB")

        train_loader = data_loader.construct_trainval_loader(cfg)
    else:
        logger.info("This is fgvc")

        #for CUB it enters this case
...
def train(cfg, args):
    ...
    logger.info(f"Classification type {cfg.DATA.MULTILABEL}")
    logger.info(f"batch={cfg.DATA.BATCH_SIZE}")
    train_loader, val_loader, test_loader = get_loaders(cfg, logger)
    logger.info("Constructing models...")
    model, cur_device = build_model(cfg)##model = ViT from vit_models.py
