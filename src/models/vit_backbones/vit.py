...
class Attention(nn.Module):
    def __init__(self, config, vis):
        ...
        self.vis = vis
        # logger.info(f"self.vis = {vis}")
        self.num_attention_heads = config.transformer["num_heads"]
        self.attention_head_size = int(config.hidden_size / self.num_attention_heads)
        self.all_head_size = self.num_attention_heads * self.attention_head_size

        # logger.info(f"Number of attention heads = {self.num_attention_heads}")
        # logger.info(f"Each attention head size = {self.attention_head_size}")
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       out_channels=config.hidden_size,#number of kernels number of patches = 16*16*3
                                       kernel_size=patch_size,
                                       stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches+1, config.hidden_size))#N_patches +1(cls token)
    ...
    def forward(self, x):
        B = x.shape[0]
        cls_tokens = self.cls_token.expand(B, -1, -1)#cls token size is (21,1,768) to attach to 21 images in a batch
...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        for layer_block in self.layer:
            hidden_states, weights = layer_block(hidden_states)#the input hidden_states are embeddings for the first block the output of this becomes input to other blocks
            if self.vis:
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis):
        ...
        # logger.info(f"From vit backbones original embeddings size is {type(self.embeddings)}")
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.cls_head = Linear(768*2,200)
        nn.init.xavier_uniform_(self.cls_head.weight)
        logger.info(f"inside backbone {config.hidden_size*2},{num_classes}")
        logger.info(f"inside backbone vision transformer")

    def forward(self, x, vis=True):
        x, attn_weights = self.transformer(x)
        print(x.size)# to know embeddings and cls token seperation
        print(x)
        print(f"from backbone {x.size}")
        ...
        with torch.no_grad():
            ...
            for bname, block in self.transformer.encoder.named_children():
                for uname, unit in block.named_children():
                    unit.load_from(weights, n_block=uname)#Load_from is from Block()
