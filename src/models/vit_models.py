...
class ViT(nn.Module):
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, load_pretrain, vis):
        ...
        logger.info(f"transfer type in vit class {transfer_type}")
        ...
        # logger.info(f"self.feat_dim = {self.feat_dim}")
        ...
        if transfer_type == "partial-1":
            ...
            for k, p in self.enc.named_parameters():
                logger.debug(f"require_grad {k}")
                if "prompt" not in k and "cls_head" not in k:
                    p.requires_grad = False
    ...
    def setup_head(self, cfg):
        if (self.cfg.MODEL.PROMPT.INCLUDE_ATTENTION_TOKEN):
            self.head = nn.Linear(self.feat_dim*2,cfg.DATA.NUMBER_CLASSES)
            # self.head = MLP(
            # input_dim=self.feat_dim*2,
            # mlp_dims=[self.feat_dim*2] * self.cfg.MODEL.MLP_NUM + \
    ...
    def forward(self, x, return_feature=False):
        ...
        if return_feature:
            return x, x
        # if (not self.cfg.MODEL.PROMPT.INCLUDE_ATTENTION_TOKEN):
        #     logger.info(f"self.head(X) {self.head}")
        #     x = self.head(x)# Not req 
        # y = 
        # logger.info(f"self.head = {self.head}")
        x=self.head(x)
        # self.head(y)
        # logger.info(f"From vit_models after calling self.enc(x) x dim = {x.shape}")
        return x #cls
